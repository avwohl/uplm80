...
class CodeGenerator:
    ...
    def _format_number(self, n: int) -> str:
        ...
        if n < 0:
            n = n & 0xFFFF
        if n > 9:
            return f"{n:04X}H" if n > 255 else f"{n:02X}H"
        return str(n)
    ...
    def generate(self, module: Module) -> str:
        ...
        # Process non-procedure declarations first (allocate storage)
        ...
        if self.needs_runtime:
            ...
            for line in runtime.split("\n"):
                if line.strip():
                    self.output.append(AsmLine(opcode=line.lstrip("\t")))
    ...
    def _gen_var_decl(self, decl: VarDecl) -> None:
        ...
        if decl.at_location:
            if isinstance(decl.at_location, NumberLiteral):
                ...
                self.data_segment.append(
                    AsmLine(label=decl.name, opcode="EQU", operands=self._format_number(addr))
                )
            else:
                # Complex AT expression - evaluate at assembly time
                self.data_segment.append(
                    AsmLine(label=decl.name, opcode="EQU", operands="$")
                )
            return
        ...
        if decl.data_values:
            ...
            self._emit_data_values(decl.data_values, decl.data_type or DataType.BYTE)
            ...
            self.data_segment.append(
                AsmLine(label=decl.name, opcode="DS", operands=str(size))
            )

    def _emit_data_values(self, values: list[Expr], dtype: DataType) -> None:
        """Emit DATA values to data segment."""
        for val in values:
            if isinstance(val, NumberLiteral):
                ...
                if name in self.literal_macros:
                    ...
                    try:
                        num_val = int(self.literal_macros[name], 0)
                        directive = "DB" if dtype == DataType.BYTE else "DW"
                        self.data_segment.append(
                ...
                for v in val.values:
                    self._emit_data_values([v], dtype)
    ...
    def _gen_proc_decl(self, decl: ProcDecl) -> None:
        ...
        # Parameters are passed on stack, accessed via offset from frame pointer
        # For simplicity, we'll copy them to local storage
        param_offset = 2  # Return address is 2 bytes
    ...
    def _gen_call_stmt(self, stmt: CallStmt) -> None:
        ...
        # Push arguments in reverse order (cdecl-like calling convention)
        for arg in reversed(stmt.args):
            arg_type = self._gen_expr(arg)
            if arg_type == DataType.BYTE:
                # Extend to 16 bits for stack
                self._emit("MOV", "L,A")
                self._emit("MVI", "H,0")
            self._emit("PUSH", "H")
    ...
    def _gen_expr(self, expr: Expr) -> DataType:
        ...
        if isinstance(expr, NumberLiteral):
            ...
            # Restore value for use in expression
            self._emit("POP", "H")
            return val_type
        ...
        if isinstance(expr, Identifier):
            ...
            if name in self.literal_macros:
                ...
                try:
                    ...
                    return DataType.BYTE if val <= 255 else DataType.ADDRESS
                except ValueError:
                    # Non-numeric literal - use as-is
                    self._emit("LXI", f"H,{macro_val}")
                    return DataType.ADDRESS
    ...
    def _gen_member_addr(self, expr: MemberExpr) -> None:
        ...
        self._gen_expr(expr.base)
        # TODO: Add proper offset based on member layout
        # For now, just return base address
    ...
    def _gen_builtin(self, name: str, args: list[Expr]) -> DataType | None:
        ...
        if name == "INPUT":
            if args:
                ...
                self._emit("MOV", "C,L")  # Port number in C
                self._emit("INP", "A")    # Input from port (use IN for 8080)
            else:
                self._emit("IN", "0")
            self._emit("MOV", "L,A")
            self._emit("MVI", "H,0")
        ...
        if name == "DEC":
            # Decrement and return previous value with flags
            self._gen_expr(args[0])
            self._emit("MOV", "A,L")
            self._emit("DCR", "A")
            self._emit("MOV", "L,A")
